...
def init_db():
    ...
    admin_email = os.getenv("ADMIN_EMAIL", "admin@cropguard.local")
    admin_password = os.getenv("ADMIN_PASSWORD", "Admin@123")
    cur.execute("SELECT id FROM users WHERE email=?", (admin_email,))
    if not cur.fetchone():
        cur.execute(
            "INSERT INTO users(name,email,password_hash,role,farm_name,location,created_at) VALUES (?,?,?,?,?,?,?)",
            ("System Admin", admin_email, generate_password_hash(admin_password), "admin", "CropGuard Control", "HQ", datetime.utcnow().isoformat()),
        )
    conn.commit()
    conn.close()
...
@app.get("/health")
def health():
    return jsonify({"status": "ok", "app": "CropGuard AI"})
